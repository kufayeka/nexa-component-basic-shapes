...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  // widgets/src/text-label-element.js
  var require_text_label_element = __commonJS({
    "widgets/src/text-label-element.js"() {
      init_lit();
      var NexaTextLabel = class extends i4 {
        static properties = {
          text: { type: String },
          color: { type: String },
  ...
  `;
        render() {
          return b2`<div style="color:${this.color}; font-size:${this.fontSize}px;">${this.text}</div>`;
        }
      };
      customElements.define("nexa-text-label", NexaTextLabel);
    }
  });
  require_text_label_element();
})();
